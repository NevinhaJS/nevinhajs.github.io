...
var Form = function (_NevinhaComponent) {
	_inherits(Form, _NevinhaComponent);
	...
	_createClass(Form, [{
		...
		value: function render() {
			...
			return h(
				...
				h('input', { placeholder: 'What needs to be done?', className: 'input', onChange: this.handleChange, type: 'text', value: name }),
				h('input', { className: 'btn submit', type: 'submit', value: 'Add todo' })
			);
		}
	}]);
...
var List = function List(_ref2) {
	...
	return h(
		'div',
		null,
		todos.map(function (_ref3, index) {
			var name = _ref3.name,
			    watched = _ref3.watched;
			return h(
				'p',
				{ key: index, className: 'data-index' },
				h(
					'div',
					{ 'data-index': index, className: 'space' },
					h(
						'div',
						{ className: 'justifc' },
						h('input', { className: 'checkbox', checked: watched, onClick: handleEdit, type: 'checkbox' }),
						h(
							'span',
							{ style: 'text-decoration: ' + (watched ? 'line-through' : 'none') + ';' },
							name
						)
					),
					h(
						'a',
						{ className: 'btn sub', href: '#', onClick: handleRemove },
...
var App = function (_NevinhaComponent2) {
	_inherits(App, _NevinhaComponent2);
	...
	function App() {
		...
		_this2.state.todos = [{ name: 'asdasda', watched: true }];
	...
	_createClass(App, [{
		key: 'handleEdit',
		value: function handleEdit(_ref4) {
			var target = _ref4.target;

			var index = target.parentNode.parentNode.dataset.index;
			var filteredTodos = this.state.todos.map(function (todo, todoIndex) {
				if (index == todoIndex) {
					return _extends({}, todo, { watched: !todo.watched });
				}
		...
		key: 'addTodo',
		value: function addTodo(_ref5) {
			var name = _ref5.name,
			    watched = _ref5.watched;

			if (!name) return;

			var todos = [].concat(_toConsumableArray(this.state.todos), [{ name: name, watched: watched }]);
		...
		value: function handleRemove(e) {
			...
			var filteredTodos = this.state.todos.filter(function (todo, todoIndex) {
